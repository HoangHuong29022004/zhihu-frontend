...
import { useDeleteFavorite } from "@/queries/favorites.query";
...
const DeleteFavorite = ({ item, setItem }: IProps) => {
  const { mutateAsync, isPending } = useDeleteFavorite(getAccessToken() ?? "");
    ...
    <>
      <Dialog open={Boolean(item)} onOpenChange={setItem}>
        <DialogContent className="sm:max-w-[425px] max-sm:w-[90%]">
          ...
          <div>
            <p>Bạn chắc chắn muốn xóa truyện khỏi yêu thích?</p>
            <InlineHint />
          </div>
...
export default memo(DeleteFavorite);
