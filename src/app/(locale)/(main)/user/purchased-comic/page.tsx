...
import { Eye, BookOpen, Clock9, PenIcon, PlayIcon } from "lucide-react";
...
import { useGetUserPurchasedComics } from "@/queries/comic.query";
...
const PurchasedComicClient: React.FC = () => {
  ...
  const { isLoading, data } = useGetUserPurchasedComics(
    getAccessToken() ?? "",
    {
      pageNumber,
      pageSize,
    ...
    <div>
      <div className="mb-6">
        <h1 className="text-lg text-text-secondary font-bold flex items-center gap-2 mb-4">
          Danh sách truyện đã mua ({data?.totalRecord || 0})
        </h1>
...
const PurchasedComic = () => {
  return (
    <Suspense fallback={<DotLoader />}>
      <PurchasedComicClient />
    </Suspense>
  );
};
