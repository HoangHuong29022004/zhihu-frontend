...
import { IAudioComic } from "@/types/audio.type";
...
const AudioSectionUser = ({ data, onDelete }: IProps) => {
  ...
  const [filteredAudios, setFilteredAudios] = useState<IAudioComic[]>(
    data.audios
  ); // Temporary using chapters data

  // Filter audios based on searchTerm
  useEffect(() => {
    if (!searchTerm) {
      setFilteredAudios(data.audios); // Temporary using chapters data
      return;
    }

    const searchValue = searchTerm.toLowerCase();
    const filtered = data.audios.filter((chapter) =>
      chapter.title?.toLowerCase().includes(searchValue)
    );
    setFilteredAudios(filtered); // Temporary using chapters data
  ...
  const handleToAudioDetails = (audioSlug: string, index: number) => {
    if (data.status === "APPROVED" || data.status == "COMPLETED") {
      router.push(
        `/comic/${data.slug}/audio/${audioSlug}?comicThumbnail=${data.thumbnail}&index=${index}&comicTitle=${data.title}`
      );
    } else {
      toast({
        variant: "destructive",
    ...
    <section className="flex flex-col gap-4 mt-6">
      ...
      <div className="flex justify-between flex-wrap gap-4 rounded-xl mb-2">
        ...
        <div className="flex gap-2">
          <Link
            href={`/user/manage-comic/add-audio?comic=${data.id}&title=${data.title}&slug=${data.slug}&lastAudio=${data.audios?.length}&salaryType=${salaryTypeSecret}`}
          >
      ...
      {/* Data table */}
      <DataTableComponent
        headers={["Tên chương", "Giá audio (dâu)", "Chi tiết"]}
        data={paginatedAudios}
        isLoading={false}
        onDelete={onDelete}
        onUpdate={(item) =>
          router.push(
            `/user/manage-comic/update-audio?audio=${item}&comicId=${data.id}&salaryType=${salaryTypeSecret}`
          )
        }
        renderCells={(row: IAudioComic) => (
          <>
            <TableCell
              className="text-text-secondary font-medium sm:max-w-24 cursor-pointer hover:text-primary transition-all duration-300"
              onClick={() => handleToAudioDetails(row.slug, row?.index ?? 0)}
            >
              {row.title || "_"}
            </TableCell>
...
export default AudioSectionUser;
