...
import RejectWithDraw from "./rejected";
import { renderPaymentStatus } from "@/utils/styling-handler";
import useMediaQuery from "@/hooks/use-screen-size";
import { useGetWithdrawAdmin } from "@/queries/withdraw.query";
import { IWithdrawAdmin } from "@/types/withdraw.type";
import { formatDateToVN } from "@/utils/time-handler";

const WithdrawClient: React.FC = () => {
  ...
  const { isLoading, data, refetch, isRefetching } = useGetWithdrawAdmin({
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex justify-between flex-wrap gap-4 bg-white px-4 py-6 rounded-xl">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <RejectWithDraw item={rejectedItem} setItem={setRejectedItem} />
        </div>
      </div>
      ...
      <DataTableComponent
        ...
        renderCells={(row: IWithdrawAdmin) => (
          <>
