...
import { useDeleteAudio } from "@/queries/audio.query";
...
const DeleteAudio = ({ item, setItem }: IProps) => {
  const { mutateAsync, isPending } = useDeleteAudio(getAccessToken() ?? "");
    ...
    <>
      <Dialog open={Boolean(item)} onOpenChange={setItem}>
        <DialogContent className="sm:max-w-[425px] max-sm:w-[90%]">
          <DialogHeader>
            <DialogTitle>Xóa audio</DialogTitle>
...
export default memo(DeleteAudio);
