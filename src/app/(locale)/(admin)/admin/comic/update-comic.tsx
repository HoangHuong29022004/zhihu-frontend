...
const UpdateComic = ({ item, setItem }: IProps) => {
  ...
  const onSubmit = async (request: TUpdateComicRequest) => {
    ...
    if (res && isSuccessResponse(res?.statusCode, res?.success)) {
      ...
      window.location.href = "/admin/comic";
    } else {
      toast({
        variant: "destructive",
  ...
  const {
    ...
    data: res,
    refetch,
  } = useGetDetailsComic(getAccessToken() ?? "", item);
  ...
  useEffect(() => {
    if (res && isSuccessResponse(res?.statusCode, res?.success)) {
      ...
      const temSalaryType = res?.data?.salaryType === "ZHIHU" ? 0 : 1;
  ...
  return (
    <>
      <Dialog open={Boolean(item && res && categoryId)} onOpenChange={setItem}>
        <DialogContent className="sm:max-w-max max-sm:w-[90%]">
          ...
          {isLoading || isLoadingGetDetails || !categoryId ? (
            <LoadingSpinner />
          ) : (
            <form
              action="post"
